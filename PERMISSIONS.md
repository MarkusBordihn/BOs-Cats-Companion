...
The Cats plugin uses **ownership-based protection** combined with **optional permission checks** for commands.
...
If LuckPerms is installed, the plugin automatically detects it on startup and logs helpful configuration tips to the server console.
...
- **Default behavior:** Without LuckPerms, ownership checks only; with LuckPerms, additional permission layer
